 ...
 */
export function libraryGetLibrarySectionHubs(
  client: PlexAPICore,
  request: operations.GetLibrarySectionHubsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    shared.MediaContainerWithHubs,
    | errors.ErrorT
    | PlexAPIError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetLibrarySectionHubsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/library/sections/{sectionId}/hubs")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, shared.MediaContainerWithHubs$inboundSchema),
