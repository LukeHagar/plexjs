 ...
 */
export function generalCreateTransientToken(
  client: PlexAPICore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.CreateTransientTokenRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.CreateTransientTokenResponse$inboundSchema),
    M.fail([400, 403, "4XX"]),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
