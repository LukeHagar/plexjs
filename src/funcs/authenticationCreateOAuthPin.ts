...
import { CreateOAuthPinServerList } from "../models/operations/createoauthpin.js";
 ...
 */
export function authenticationCreateOAuthPin(
  client: PlexAPICore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.CreateOAuthPinRequest$outboundSchema.parse(value),
  ...
  const baseURL = options?.serverURL
    || pathToFunc(CreateOAuthPinServerList[0], { charEncoding: "percent" })();
  ...
  const requestSecurity = resolveSecurity(
    [
      {
        fieldName: "X-Plex-Client-Identifier",
        type: "apiKey:header",
        value: security?.clientIdentifier,
      },
    ],
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.CreateOAuthPinResponse$inboundSchema),
