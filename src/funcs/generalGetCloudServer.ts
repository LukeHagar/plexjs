...
import { GetCloudServerServerList } from "../models/operations/getcloudserver.js";
 ...
 */
export function generalGetCloudServer(
  client: PlexAPICore,
  request: operations.GetCloudServerRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    shared.CloudServerResponse,
    | errors.ErrorT
    | PlexAPIError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetCloudServerRequest$outboundSchema.parse(value),
  ...
  const baseURL = options?.serverURL
    || pathToFunc(GetCloudServerServerList[0], { charEncoding: "percent" })();

  const path = pathToFunc("/cloud_server")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, shared.CloudServerResponse$inboundSchema),
