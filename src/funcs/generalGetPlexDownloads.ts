...
import { GetPlexDownloadsServerList } from "../models/operations/getplexdownloads.js";
 ...
 */
export function generalGetPlexDownloads(
  client: PlexAPICore,
  request: operations.GetPlexDownloadsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    shared.PlexDownloadsResponse,
    | errors.ErrorT
    | PlexAPIError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetPlexDownloadsRequest$outboundSchema.parse(value),
  ...
  const baseURL = options?.serverURL
    || pathToFunc(GetPlexDownloadsServerList[0], { charEncoding: "percent" })();
  ...
  const path = pathToFunc("/downloads/{channel}.json")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, shared.PlexDownloadsResponse$inboundSchema),
