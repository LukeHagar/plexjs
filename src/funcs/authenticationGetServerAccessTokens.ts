...
import { GetServerAccessTokensServerList } from "../models/operations/getserveraccesstokens.js";
 ...
 */
export function authenticationGetServerAccessTokens(
  client: PlexAPICore,
  request: operations.GetServerAccessTokensRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    shared.ServerAccessTokensResponse,
    | errors.ErrorT
    | PlexAPIError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetServerAccessTokensRequest$outboundSchema.parse(value),
  ...
  const baseURL = options?.serverURL
    || pathToFunc(GetServerAccessTokensServerList[0], {
      charEncoding: "percent",
    })();

  const path = pathToFunc("/server/access_tokens")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, shared.ServerAccessTokensResponse$inboundSchema),
