...
import { ExchangeJWTTokenServerList } from "../models/operations/exchangejwttoken.js";
 ...
 */
export function authenticationExchangeJWTToken(
  client: PlexAPICore,
  request: operations.ExchangeJWTTokenRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    shared.AuthTokenResponse,
    | errors.ErrorT
    | PlexAPIError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.ExchangeJWTTokenRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.TokenExchangeRequest, {
    explode: true,
  });

  const baseURL = options?.serverURL
    || pathToFunc(ExchangeJWTTokenServerList[0], { charEncoding: "percent" })();

  const path = pathToFunc("/auth/token")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, shared.AuthTokenResponse$inboundSchema),
