...
import { GetServerDetailsServerList } from "../models/operations/getserverdetails.js";
 ...
 */
export function usersGetServerDetails(
  client: PlexAPICore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetServerDetailsRequest$outboundSchema.parse(value),
  ...
  const baseURL = options?.serverURL
    || pathToFunc(GetServerDetailsServerList[0], { charEncoding: "percent" })();

  const pathParams = {
    machineId: encodeSimple("machineId", payload.machineId, {
      explode: false,
      charEncoding: "percent",
    }),
  };
  const path = pathToFunc("/servers/{machineId}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, shared.ServerConfiguration$inboundSchema),
