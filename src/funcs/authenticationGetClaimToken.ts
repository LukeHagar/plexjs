...
import { GetClaimTokenServerList } from "../models/operations/getclaimtoken.js";
 ...
 */
export function authenticationGetClaimToken(
  client: PlexAPICore,
  request: operations.GetClaimTokenRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    shared.ClaimTokenResponse,
    | errors.ErrorT
    | PlexAPIError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetClaimTokenRequest$outboundSchema.parse(value),
  ...
  const baseURL = options?.serverURL
    || pathToFunc(GetClaimTokenServerList[0], { charEncoding: "percent" })();

  const path = pathToFunc("/claim/token.json")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, shared.ClaimTokenResponse$inboundSchema),
