...
import { GetServerUserFeaturesServerList } from "../models/operations/getserveruserfeatures.js";
 ...
 */
export function usersGetServerUserFeatures(
  client: PlexAPICore,
  request: operations.GetServerUserFeaturesRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    shared.ServerUserFeaturesResponse,
    | errors.ErrorT
    | PlexAPIError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetServerUserFeaturesRequest$outboundSchema.parse(value),
  ...
  const baseURL = options?.serverURL
    || pathToFunc(GetServerUserFeaturesServerList[0], {
      charEncoding: "percent",
    })();

  const path = pathToFunc("/server/users/features")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, shared.ServerUserFeaturesResponse$inboundSchema),
