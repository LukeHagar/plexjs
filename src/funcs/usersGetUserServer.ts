...
import { GetUserServerServerList } from "../models/operations/getuserserver.js";
 ...
 */
export function usersGetUserServer(
  client: PlexAPICore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetUserServerRequest$outboundSchema.parse(value),
  ...
  const baseURL = options?.serverURL
    || pathToFunc(GetUserServerServerList[0], { charEncoding: "percent" })();

  const path = pathToFunc("/server")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, shared.ConnectionInfo$inboundSchema),
