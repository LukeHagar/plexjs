...
import { RegisterDeviceJWKServerList } from "../models/operations/registerdevicejwk.js";
 ...
 */
export function authenticationRegisterDeviceJWK(
  client: PlexAPICore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.RegisterDeviceJWKRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.JWKRegistrationRequest, {
    explode: true,
  });

  const baseURL = options?.serverURL
    || pathToFunc(RegisterDeviceJWKServerList[0], {
      charEncoding: "percent",
    })();

  const path = pathToFunc("/auth/jwk")();
