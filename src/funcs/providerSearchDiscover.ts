...
import { SearchDiscoverServerList } from "../models/operations/searchdiscover.js";
 ...
 */
export function providerSearchDiscover(
  client: PlexAPICore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.SearchDiscoverRequest$outboundSchema.parse(value),
  ...
  const baseURL = options?.serverURL
    || pathToFunc(SearchDiscoverServerList[0], { charEncoding: "percent" })();

  const path = pathToFunc("/library/search")();
  ...
  const query = encodeFormQuery({
    ...
    "searchTypes": payload.searchTypes,
  });
