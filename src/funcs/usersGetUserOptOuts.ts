...
import { GetUserOptOutsServerList } from "../models/operations/getuseroptouts.js";
 ...
 */
export function usersGetUserOptOuts(
  client: PlexAPICore,
  request: operations.GetUserOptOutsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    shared.UserOptOutsResponse,
    | errors.ErrorT
    | PlexAPIError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetUserOptOutsRequest$outboundSchema.parse(value),
  ...
  const baseURL = options?.serverURL
    || pathToFunc(GetUserOptOutsServerList[0], { charEncoding: "percent" })();
  ...
  const path = pathToFunc("/user/{uuid}/settings/opt_outs")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, shared.UserOptOutsResponse$inboundSchema),
